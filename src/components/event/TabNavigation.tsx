...
const TabNavigation: React.FC<TabNavigationProps> = ({ tabConfigItems, tabSettings, t, activeTab, setActiveTab }) => {
  return (
    <TabsList className="flex w-full flex-nowrap overflow-x-auto gap-1 bg-white/90 border-b border-gray-200">
      <TabsTrigger
        value="settings"
        className={`min-w-[64px] px-0.5 md:min-w-[160px] md:px-6 whitespace-nowrap${activeTab === 'settings' ? ' bg-blue-100 font-bold text-blue-700' : ''}`}
      ...
      <TabsTrigger
        value="basic"
        className={`min-w-[64px] px-0.5 md:min-w-[160px] md:px-6 whitespace-nowrap${activeTab === 'basic' ? ' bg-blue-100 font-bold text-blue-700' : ''}`}
      ...
      {tabConfigItems.map(item => {
        ...
        return (
          <TabsTrigger
            key={item.key}
            value={item.key}
            className={`min-w-[64px] px-0.5 md:min-w-[160px] md:px-6 whitespace-nowrap${isActive ? ' bg-blue-100 font-bold text-blue-700' : ''}`}
