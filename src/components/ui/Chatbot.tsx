  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessages((prev) => [...prev, { from: 'bot', text: err.error || 'Bot error!' }]);
        setLoading(false);
        return;
      }
          ...
          <div className="flex-1 px-4 py-3 bg-gray-50 overflow-y-auto" style={{ maxHeight: 480 }}>
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex mb-2 ${msg.from === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`px-3 py-2 rounded-xl text-sm max-w-[75%] ${msg.from === 'bot' ? 'bg-white text-gray-800 border border-purple-100' : 'bg-gradient-to-br from-purple-600 to-indigo-600 text-white'}`}>
                  {msg.text}
                </div>
                {msg.from === 'user' && (
